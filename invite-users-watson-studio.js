...
async function addUsersToWatsonStudioProject(project_id, role, filtered_users) {
  ...
  -H 'cache-control: no-cache' -d '{
    ...
    */
    ...
    for(const filtered_user of filtered_users) {
        const members = [{
                user_name : filtered_user.email,
                role: role,
                id: filtered_user.iam_id
        }]
       
    
    
        const url = `https://api.dataplatform.cloud.ibm.com/v2/projects/${project_id}/members`
        const options = {
            json: true,
            uri: url,
            method: 'POST',
        ...
        try {
            ...
            console.log("[addUsersToWatsonStudioProject] Response: " + JSON.stringify(response))
        } catch(e) {
            console.error("[addUsersToWatsonStudioProject] Error: " + e.message);
        }
